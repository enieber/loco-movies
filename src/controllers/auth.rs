...
async fn register(
    ...
    AuthMailer::send_welcome(&ctx, &user).await?;
     ...
     format::render().redirect_with_header_key("HX-Redirect", "/auth/login")
}
...
async fn login(
    ...
    tracing::info!("generate token");
    // Criar o cookie HTTP Only
    ...
    let user_response = views::auth::LoginResponse::new(&user, &token);
...
async fn current(auth: auth::JWT, State(ctx): State<AppContext>) -> Result<Response> {
    ...
    format::json(views::auth::CurrentResponse::new(&user))
}
...
async fn magic_link_verify(
    ...
    format::json(views::auth::LoginResponse::new(&user, &token))
}

pub fn routes() -> Routes {
    Routes::new()
        .prefix("/auth")
        .add("/login", get(login_view))
        .add("/login", post(login))
        .add("/register", get(register_view))
